...
const getJavascriptLoaderConfig = ({
  ...
  options: {
    ...
    presets: [
      ...
      [
        "@babel/preset-react",
        { development: isProcessingJSXFiles && isEnvDevelopment },
      ], // `development` flag allows accurate source code location
  ...
  const config = {
    ...
    plugins: [
      ...
      new ForkTsCheckerWebpackPlugin({
        ...
        eslint: {
          enabled: true,
          files: ["src/**/*.ts", "src/**/*.tsx", "style/**/*.ts"],
          options: {
            // See https://eslint.org/docs/developer-guide/nodejs-api#cliengine
            configFile:
              isEnvProduction || enableAdvancedMode
                ? paths.advancedEslintConfig
                : paths.eslintConfig,
          },
        },
